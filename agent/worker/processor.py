...
async def _handle_failure(rid: str, req: dict, result: dict, retry_after: dict = None):
    ...
    # A capability the batch path does not have, a missing Flow project, or a bad
    # model setting is a configuration answer — not something a retry can reach.
    if any(code in error_lower for code in ("unsupported_on_batch_api", "no_flow_project", "invalid_model_config")):
