...
| Clip has people talking in it / two voices over one scene | The clip generated its own dialogue on top of the TTS narration. Check the project: `GET /api/projects/<PID>` → `allow_voice` must be `0`. If it is `1`, `PATCH /api/projects/<PID> -d '{"allow_voice": false}'` and regenerate those videos. Then check the scene's `video_prompt` for dialogue verbs (*says, asks, whispers, shouts, replies*) — rewrite them as action and put the words in `narrator_text`. Agents before this fix also dropped the whole `Audio:` ban when `allow_music` was on, so a project with music and no speech ban needs the agent updated too |
